...
# ArrayTreeMapping = RecursiveMapping[str, Array]
ArrayTreeMapping = Mapping[str, Union['ArrayTreeMapping', Array]]
# ArrayTree = Union[RecursiveMapping[str, Array], Array]
...
def kaiming_init(sd: float, shape: Tuple[int, ...]) -> Array:
    """
...
def embedding_init(scale: float, shape: Tuple[int, ...]) -> Array:
    """
    Arguments:
...
def normal_init(sd: float, shape: Tuple[int, ...]) -> Array:
    return xp.array(np.random.normal(0, sd, shape))


def array_gen(params: ArrayParams) -> Array:
